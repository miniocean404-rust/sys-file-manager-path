...
pub unsafe fn get_explore_root_window() -> Option<HWND> {
    ...
    if let Ok(cabinet_hwnd) = unsafe { FindWindowW(PCWSTR(class_name.as_ptr()), None) } {
    ...
    if let Ok(explore_hwnd) = unsafe { FindWindowW(PCWSTR(class_name.as_ptr()), None) } {
...
pub unsafe fn get_window_program_path(window: HWND) -> String {
    ...
    let path_len = unsafe { GetWindowModuleFileNameW(window, &mut path) };
...
pub unsafe fn get_window_exec_path(window: HWND) -> anyhow::Result<String> {
    ...
    let len = unsafe { GetModuleFileNameExW(Some(process_handle), None, &mut buffer) };
...
pub unsafe extern "system" fn enum_windows(window: HWND, _: LPARAM) -> bool {
    let info = unsafe { get_window_info(window).unwrap() };
    let title = unsafe { get_window_title(window) };
    let path = unsafe { get_window_program_path(window) };
...
pub unsafe extern "system" fn enum_child_proc(window: HWND, _: LPARAM) -> bool {
    let classname = unsafe { get_window_title_classname(window) };
